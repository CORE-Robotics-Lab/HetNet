...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        self.rnn_states_critic = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents,
                                           self.recurrent_N, self.hidden_size), dtype=np.float32)
    ...
    def sample_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        rnn_states_batch = self.rnn_states[:-1].reshape(-1, 2*(self.args.num_A + self.args.num_P), 128)
        rnn_obs_batch = self.rnn_obs[:-1].reshape(-1, 2*(self.args.num_P), 16)


        # obs_batch = self.obs[:-1].reshape(-1, *self.obs.shape[3:])
        # rnn_states_batch = self.rnn_states[:-1].reshape(-1, *self.rnn_states.shape[3:])
        # rnn_obs_batch = self.rnn_obs[:-1].reshape(-1, *self.rnn_obs.shape[3:])
        #
