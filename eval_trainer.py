...
class EvalTrainer(object):
    ...
    def run_batch(self, epoch):
        ...
        print('std steps taken for ', self.args.eval_string, 'is :',
              np.std(steps_taken_array) / np.sqrt(len(steps_taken_array)))
        with open(self.args.eval_string + '_steps_taken.txt', 'w') as output:
            output.write('steps_taken=[')
            for item in steps_taken_array:
                output.write("%s," % item)
            output.write(']')
        with open(self.args.eval_string + '_reward.txt', 'w') as output:
            output.write('reward=[')
            for item in reward_array:
                output.write("%s\n," % item)
            output.write(']')
            # output.write(str(steps_taken_array))
        d = dict()
        d['reward'] = reward_array
        print(torch.mean(torch.tensor(reward_array), dim=0))
        torch.save(d,self.args.eval_string + '_reward.pt')
        exit(0)
    ...
    def train_batch(self, epoch):
        ...
        # will exit before any of the stuff below
        if self.args.hetcomm:
