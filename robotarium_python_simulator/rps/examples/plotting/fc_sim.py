...
pred1_locations = [[2, 0], [2, 1], [2, 2], [2, 3], [2, 4], [2, 4], [2, 4], [1, 4], [1, 4], [0, 4], [0, 4], [1, 4], [0, 4], [0, 4], [0, 4], [1, 4], [1.03, 4], [2, 4], [3, 4], [3, 4], [3, 4], [1, 3]]
pred2_locations = [[0, 2], [0, 3], [0, 3], [1, 3], [1, 4], [1, 4], [1, 4], [0, 4], [0, 4], [0, 4], [0, 4], [1, 4], [0, 4], [0, 4], [0, 4], [1, 4], [.98, 4], [2, 4], [3, 4], [3, 4], [3, 4], [3, 1]]
...
for e, (coord0, coord1) in enumerate(pred1_locations):
    pred1_locations[e][0] = coord0 - 2.5
    pred1_locations[e][1] = coord1 - 2.5

for e, (coord0, coord1) in enumerate(pred2_locations):
    pred2_locations[e][0] = coord0 - 2.5
    pred2_locations[e][1] = coord1 - 2.5

for e, (coord0, coord1) in enumerate(action1_locations):
    action1_locations[e][0] = coord0 - 2.5
    action1_locations[e][1] = coord1 - 2.5
...
N = 3
scaling_factor = .5
# initial_conditions = np.zeros((3,3))
# initial_conditions[:2,:] = scaling_factor * np.array([pred1_locations[0], pred2_locations[0], action1_locations[0]]).T #np.array(np.mat('1 0.5 -0.5 0; 0.8 -0.3 -0.75 0.1; 0 0 0 0'))
#
initial_conditions = create_goal_points(0)
r = robotarium.Robotarium(number_of_robots=N, show_figure=True, initial_conditions=initial_conditions, sim_in_real_time=False)

# Define goal points by removing orientation from poses
goal_points = np.zeros((3,3))
goal_points[:2,:] = scaling_factor * np.array([pred1_locations[1], pred2_locations[1], action1_locations[1]]).T
# goal_points = 0.9*np.array([[-1, 1, -1],[-1, -1, 1],[0,0,0]])
...
for i in range(1, 21):
    ...
    print(goal_points)
    while (np.size(at_pose(np.vstack((x_si,x[2,:])), goal_points, position_error=.15, rotation_error=100)) != N):
